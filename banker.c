...
int main() {
    ...
    while (choice != 7) {
        ...
        switch (choice) {
            ...
            case 6: {
                int process;
                printf("Enter the process requesting resources (0-indexed): ");
                scanf("%d", &process);
...
void resourceRequest(int process) {
    ...
    if (granted) {
        ...
        if (isSafeState()) {
            printf("\nResource request granted. System is in a safe state.\n");
            printf("Safe sequence: ");
            for (int i = 0; i < num_processes; i++)
                printf("%d ", safeSequence[i]);
            printf("\n");
        } else {
            printf("\nResource request granted but the system becomes unsafe. Rolling back changes.\n");
            for (int i = 0; i < num_resources; i++) {
                available[i] += request[i];
                allocation[process][i] -= request[i];
                need[process][i] += request[i];
        ...
        printf("\nResource request denied. Process must wait since resources are not available.\n");
    }
}